...
#!
    def self.complaint_text
        return "Пожалуйста, кратко изложите ситуацию. \n\nУбедитесь, что ваше описание кратко и находится в диапазоне от #{ENV['MIN_LENGTH_COMPLAINT_TEXT']} до #{ENV['MAX_LENGTH_COMPLAINT_TEXT']} символов." if $lg == Ru 
        return "Please provide a brief explanation of the situation. \n\nEnsure your description is succinct and falls within #{ENV['MIN_LENGTH_COMPLAINT_TEXT']} to #{ENV['MAX_LENGTH_COMPLAINT_TEXT']} characters." if $lg == En 
        return "Por favor, proporciona una breve explicación de la situación. \n\nAsegúrate de que tu descripción sea concisa y esté entre #{ENV['MIN_LENGTH_COMPLAINT_TEXT']} y #{ENV['MAX_LENGTH_COMPLAINT_TEXT']} caracteres." if $lg == Es 
        return "请简要说明情况。 \n\n请确保您的描述简洁并且在#{ENV['MIN_LENGTH_COMPLAINT_TEXT']}至#{ENV['MAX_LENGTH_COMPLAINT_TEXT']}个字符之间。" if $lg == Cn
    end
#!
    def self.more_then_max_length
        return "🗒 Вы превысили лимит длины сообщения, отправьте сообщение длиной до #{ENV['MAX_LENGTH_COMPLAINT_TEXT']} символов" if $lg == Ru 
...
#!
    def self.less_then_min_length
        return "📝 Вы ввели слишком короткое сообщение. Введите сообщение длиной минимум #{ENV['MIN_LENGTH_COMPLAINT_TEXT']} символов" if $lg == Ru 
...
#!
    def self.become_scamer
        return "💔💔💔 Вы были обнаружены в списке кидков!  💔💔💔 \n\n🚸 Опротестуйте решение модератора, чтобы восстановить своё честное имя и опубликовать опровержение!" if $lg == Ru 
        return "💔💔💔 You have been found on the list of rippers! 💔💔💔 \n\n🚸 Appeal the moderator's decision to restore your honest name and post a rebuttal!" if $lg == En 
        return "💔💔💔 ¡Has sido encontrado en la lista de rippers! 💔💔💔 \n\n🚸 ¡Apela la decisión del moderador para restaurar tu nombre honesto y publicar una refutación!" if $lg == Es 
...
#!
    def self.require_subscribe_channel
        ...
        return "☢️ It is necessary to subscribe to the <a href='#{ENV['TELEGRAM_CHANNEL_USERNAME']}'>channel</a>." if $lg == En 
        return "☢️ Es necesario suscribirse al <a href='#{ENV['TELEGRAM_CHANNEL_USERNAME']}'>canal</a>." if $lg == Es 
        return "☢️ 需要订阅该频<a href='#{ENV['TELEGRAM_CHANNEL_USERNAME']}'>道</a>。" if $lg == Cn
...
#!
    def self.verifyed users_data
        ...
        return "#{users_data}  - ⚜️ 已验证" if $lg == Cn
        # проверенный
    end
#!
...
#!
    def self.verifying_user user, status, data = nil
        formatted_status =
            ...
            elsif $lg == En && status == 'verified';   '⚜️ Verified.'
            elsif $lg == En && status == 'trusted';    '🔱 trusted.'
            elsif $lg == En && status == 'dwc';        '☢️ DWC.'

            elsif $lg == Es && status == 'scamer';     '🚫 Ripper.'
            elsif $lg == Es && status == 'not_scamer'; '✅ No es una ripper.'
            elsif $lg == Es && status == 'verified';   '⚜️ Verificado.'
            elsif $lg == Es && status == 'trusted';    '🔱 trusted.'
            elsif $lg == Es && status == 'dwc';        '☢️ Tratar con precaución.'
...
#!
    def self.support
        return "<b>Вы можете связаться с поддержкой Oracle для решения экстренных вопросов, подачи заявлений на верификацию, по вопросам сотрудничества и прочему. \n\nПожалуйста, подавайте ваши жалобы с помощью интерфейса бота, а не через службу поддержки.</b>" if $lg == Ru 
        return "<b>You can contact Oracle support for urgent matters, to apply for verification, for collaboration inquiries, and more. \n\nPlease submit your reports using the bot interface, not through the support service.</b>" if $lg == En 
        return "<b>Puede ponerse en contacto con el soporte de Oracle para asuntos urgentes, para solicitar verificación, para consultas de colaboración y más. \n\nPor favor, envíe sus informes utilizando la interfaz del bot, no a través del servicio de soporte.</b>" if $lg == Es 
        return "<b>您可以联系Oracle支持解决紧急问题、申请验证、合作咨询等。\n\n请通过机器人界面提交您的报告，而不是通过支持服务。</b>" if $lg == Cn
        # 'Связаться c поддержкой'
    end
#!
    def self.oracle_tips
        return "<b>Каждый день мы получаем десятки репортов о кидке. Но по-настоящему изящных кидков из них не более 10%. Остальные 90% кидков работают по древним и тупым схемам. \n\nВы можете защить себя, подробно изучив наши инструкции и советы по безопасной работе в Telegram в разделе Oracle's Tips.</b>" if $lg == Ru
        return "<b>Every day, we receive dozens of scam reports. However, only about 10% of them are truly sophisticated scams. The other 90% operate on old and dull schemes. \n\nYou can protect yourself from these by thoroughly studying our instructions and safety tips for working in Telegram under the Oracle's Tips section.</b>" if $lg == En
        return "<b>Todos los días recibimos decenas de informes de estafas. Sin embargo, solo alrededor del 10% de ellos son estafas verdaderamente sofisticadas. El otro 90% opera con esquemas antiguos y simplones. \n\nPuede protegerse de estos estudiando detalladamente nuestras instrucciones y consejos de seguridad para trabajar en Telegram en la sección Consejos de Oracle." if $lg == Es
        return "<b>我们每天都收到数十份有关欺诈的报告。但其中真正精细的欺诈只占大约10%。其他90%的欺诈都是使用古老且愚蠢的手段。 \n\n您可以通过仔细研究我们在Oracle's Tips部分的Telegram安全工作指南和建议来保护自己免受这些欺诈的侵害。</b>" if $lg == Cn
    end
#!
    def self.option_details
        return '👹 <b>Пожалуйста, прикрепите всю доступную вам информацию о человеке, включая его платежные реквизиты, ники, имена, голосовые сообщение, IP адреса, скриншоты его экрана, отправленные им кружочки, и прочую информацию.</b>' if $lg == Ru
        return '👹 <b>Please provide all available information about the person, including their payment details, nicknames, names, voice messages, IP addresses, screenshots of their screen, circles sent by them, and other information.</b>' if $lg == En
        return '👹 <b>Por favor, adjunte toda la información disponible que tenga sobre la persona, incluyendo sus datos de pago, apodos, nombres, mensajes de voz, direcciones IP, capturas de pantalla de su pantalla, los círculos enviados por él y otra información relevante.</b>' if $lg == Es
        return '👹 <b>请提供您所掌握的有关此人的所有信息，包括支付数据、昵称、姓名、语音消息、IP地址、屏幕截图、他发送的圆圈等其他相关信息。</b>' if $lg == Cn
    end

#!
    def self.to_userbot
        return "⛳️ Пожалуйста, выберите чат с человеком, которого хотите проверить. Так же, вы можете отправь ссылку на аккаунт или ІD." if $lg == Ru 
        return "⛳️ Please select the chat with the person you want to check. Also, you can send a link to the account or ID." if $lg == En 
        return "⛳️ Por favor, seleccione el chat con la persona que desea verificar. También puede enviar un enlace a la cuenta o ID." if $lg == Es 
        return "⛳️ 请选择您想要检查的人的聊天。您也可以发送账户链接或ID。" if $lg == Cn
    end
#!
    def self.checking
...
#!
    def self.notify_already_has_requesting_complaint
        ...
        return "🥴 A complaint has already been filed against the selected user.\n\nCurrently, it is under review. If it is rejected by the moderator, you will have the opportunity to submit a new complaint." if $lg == En
        return "🥴 Ya se ha presentado una queja contra el usuario seleccionado.\n\nActualmente, está en revisión. Si es rechazada por el moderador, tendrás la oportunidad de presentar una nueva queja." if $lg == Es
...
#!
    def self.notify_already_scammer_status
        return "Выбранный пользователь находится в списке скамеров.\n\nНовые жалобы можно будет отправлять, после того как текущая претензия будет снята." if $lg == Ru
        return "The selected user is on the scammers list.\n\nYou will be able to file new complaints after the current claim is lifted." if $lg == En
        return "El usuario seleccionado está en la lista de estafadores.\n\nPodrás presentar nuevas quejas una vez que se retire la reclamación actual." if $lg == Es
...
#!
    def self.media_data_getted
        ...
        return "✍🏽 Your information has been received. You can send additional information or click the 'That`s it' button." if $lg == En
        return "✍🏽 Hemos recibido tu información. Puedes enviar información adicional o hacer clic en el botón 'Listo'." if $lg == Es
